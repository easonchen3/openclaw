...
function buildRealtimeInstructions(): string {
  return `You are NOEMate's realtime voice interface. Keep spoken replies concise. If the user asks for code, repository state, tools, files, current NOEMate context, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`;
}
