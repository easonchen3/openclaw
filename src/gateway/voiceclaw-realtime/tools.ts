...
export function buildAsyncToolAck(toolName: string): string {
  return JSON.stringify({
    ...
    tool: toolName,
    message:
      "The NOEMate tool is running asynchronously. Do not answer with final results yet; wait for the injected tool result.",
  });
}
...
function buildUntrustedToolContext(params: {
  ...
  return [
    ...
    "Tool update JSON:",
    payloadText,
    "End of NOEMate async tool update.",
    params.guidance,
  ].join("\n\n");
}
