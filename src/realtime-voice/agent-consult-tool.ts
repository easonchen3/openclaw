...
export const REALTIME_VOICE_AGENT_CONSULT_TOOL: RealtimeVoiceTool = {
  ...
  description:
    "Ask the full NOEMate agent for deeper reasoning, current information, or tool-backed help before speaking.",
  parameters: {
    type: "object",
    properties: {
      question: {
...
export function buildRealtimeVoiceAgentConsultWorkingResponse(
  ...
  return {
    ...
    message: `Tell the ${audienceLabel} briefly that you are checking, then wait for the final NOEMate result before answering with the actual result.`,
  };
}
...
export function buildRealtimeVoiceAgentConsultPrompt(params: {
  ...
  return [
    `You are helping a NOEMate realtime voice agent during ${params.surface}.`,
